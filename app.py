...
# --- 2. DICTIONNAIRE DE RECHERCHE STRATÉGIQUE (CONSERVÉ À L'IDENTIQUE) ---
...
# --- 3. DESIGN (CONSERVÉ À L'IDENTIQUE) ---
st.markdown("""
    <style>
...
# --- 4. MOTEUR IA (CONSERVÉ À L'IDENTIQUE) ---
def traiter_ia_expert(liste_brute, service):
    ...
    prompt = f"Trie ces articles pour {service}. Supprime les doublons. Garde les 4 plus PERTINENTS (URLs seules) :\n{titres_concat}"
    try:
...
# --- 6. EXECUTION (CORRECTION DE L'ERREUR MOMENTANÉE UNIQUEMENT) ---
if st.button("LANCER LA VEILLE INTELLIGENTE 🚀", use_container_width=True):
    for sujet in st.session_state['sujets']:
        ...
        for attempt in range(2): # On tente 2 fois si besoin
            try:
                with st.spinner(f"Recherche stratégique pour {sujet}..."):
                    with DDGS() as ddgs:
                        # On garde 25 résultats pour que l'IA ait du choix (PERTINENCE)
                ...
                if attempt == 0:
                    time.sleep(4) # Pause de sécurité avant de réessayer
                continue
        
        # Pause obligatoire entre chaque service pour ne pas saturer DuckDuckGo
        time.sleep(1.8)
        ...
        if success:
            ...
            st.error(f"Le service {sujet} est momentanément saturé. Les autres services continuent...")

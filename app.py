...
if st.button("LANCER LA VEILLE INTELLIGENTE 🚀", use_container_width=True):
    ...
    for sujet in st.session_state['sujets']:
        ...
        for attempt in range(2):
            try:
                with st.spinner(f"Recherche pour {sujet}..."):
                    with DDGS() as ddgs:
        ...
        if success:
            ...
            st.session_state['last_results'][sujet] = actus # Stockage pour le PDF
            
            col1, col2 = st.columns([1, 1.4])
            with col1:
                st.markdown(f'<div class="analyse-box">💡 <b>{message_ia}</b></div>', unsafe_allow_html=True)
            with col2:
                if not actus: st.info("Pas d'actualité unique identifiée.")
                for a in actus:
            ...
            st.error(f"Le service {sujet} est momentanément saturé.")
    
    # Force le rafraîchissement pour afficher le bouton PDF en haut après la veille
    st.rerun()

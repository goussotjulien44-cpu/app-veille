...
# --- 3. CLASSE GÉNÉRATION PDF (ADAPTÉE NOUVELLE STRUCTURE) ---
class PyxisPDF(FPDF):
    def header(self):
...
def generer_pdf(resultats_complets):
    ...
    # resultats_complets est maintenant un dict : {'Service': {'articles': [], 'analysis': 'txt'}}
...
def traiter_ia_expert(liste_brute, service):
    ...
    Consigne de dédoublonnage radicale : Si plusieurs articles parlent du même événement, n'en garde qu'UN SEUL (le plus complet).
...
# --- 7. LOGIQUE D'AFFICHAGE ET D'ACTION (SÉPARÉE) ---

# A. BOUTON D'ACTION (Ne fait que chercher et stocker)
if st.button("LANCER LA VEILLE INTELLIGENTE 🚀", use_container_width=True):
    ...
    for sujet in st.session_state['sujets']:
        with st.status(f"Analyse en cours : {sujet}...", expanded=False) as status:
            ...
            time.sleep(1.5) # Petite pause antiban

            if success:
                actus, message_ia = traiter_ia_expert(raw, sujet)
                # ON STOCKE TOUT : Articles ET Message IA
    ...
    st.rerun() # On recharge pour passer à l'affichage propre

# B. ZONE D'AFFICHAGE PERSISTANTE (S'affiche si données présentes)
if st.session_state['last_results']:
    ...
    # 2. L'affichage des cartes (C'est ce qui manquait !)
    for sujet, data in st.session_state['last_results'].items():
        ...
        with col2:
            if not actus:
                st.info("Aucun article unique identifié.")
            for a in actus:
